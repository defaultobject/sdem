...
def start_omniboard(experiment_name):
    ...
    omniboard_config_dict = {
        "sacred_1": {
            "mongodbURI": "mongodb://{user}:{password}@{host}/{db_name}".format(
                password=password, user=user, host=host, db_name=db_name
            ),
...
def start_sacredboard(experiment_name):
    ...
    os.system(
        "sacredboard -mu {uri} {db_name} -mc {name}".format(
            uri=mongodbURI, db_name=db_name, name=experiment_name
        )
    )
