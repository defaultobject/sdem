...
def sync():
    ...
    experiment_folders = sacred_manager.order_experiment_folders_by_datetime(experiment_folders)
    ...
    for _id in experiment_folders:
        ...
        try:
            ...
            if state.verbose:
                logger.info(f'Error getting experiment _id from experient run {_id} - ignoring!')
            #raise e
        ...
        try:
            storage_converter.file_storage_to_mongo_db(experiment_name, collection, 'models/runs', _id, insert_id, overwrite=True)
        except:
            ...
            if state.verbose:
                logger.info(f'There was a problem with experiment {experiment_name} inserting error entry {_id}')

            storage_converter.file_storage_to_mongo_db(experiment_name, collection, 'models/runs', _id, insert_id, overwrite=True, error_entry=True)
